...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            // for Professor or Student, get department ID
            var query =
                from t in db.Departments
                where t.Subject == departmentAbbrev
                select t.DepartId;
            var temp = query.ToList();

            // have duplicate answer, will not allow none.
            if (temp.Count() != 1)
            {
                Debug.WriteLine(temp.Count());
                throw new ArgumentException("duplicate department or department not exist.");
            }

            // set defult department for new user 
            uint departId = temp[0];
            if (role == "Student")
            {
        ...
        private string GenerateUniqueId()
        {
            ...
            while (match != 0)
            {
                uniqueId = "u" + random.Next(1000000, 10000000).ToString();
                Debug.WriteLine("random get: " + uniqueId);

                var inAdmin = from t in db.Administrators where t.UId == uniqueId select t;
                var inStudent = from t in db.Students where t.UId == uniqueId select t;
                var inProfes = from t in db.Professors where t.UId == uniqueId select t;
                match = inAdmin.ToList().Count() + inProfes.ToList().Count() + inStudent.ToList().Count();
            }
            return uniqueId;
        }
        /*******End code to modify********/
